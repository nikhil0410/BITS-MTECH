...
2. **Custom BPE Tokenizer** (vocab_size=10,000)  
   - Trained from scratch on the cleaned SQuAD corpus using Byte Pair Encoding (BPE). This captures domain?specific subword units (e.g., "Napoleon", "Paris", "Revolution") and reduces out?of?vocabulary tokens.  
   ...
   - **Important:** The vocabulary contains subword tokens, not full words. Common words may be split into multiple subword units by BPE merges. Therefore, evaluation is performed on subword tokens.
...
3. **Model Architecture** (from scratch)  
   ...
   - This size balances representational power with feasible training on available compute resources (executed on BITS Kubeflow Remote Lab).

4. **Training Setup**  
   - **Objective**: Masked Language Modeling – 15% of tokens masked per batch (80% ? `[MASK]`, 10% ? random token, 10% unchanged).  
   - **Optimizer**: AdamW (lr=5e-4, weight decay=0.01) with linear warmup over the first 2 epochs (start_factor=0.1).  
   - **Batch size**: 32, sequence length: 128 tokens.  
   - **Gradient clipping**: 1.0 to stabilise training.

## Challenges Faced

- **Long training time**: The full SQuAD corpus contains ~590,000 sentences. With batch size 32, each epoch processed ~16,700 batches ? ~30–40 minutes per epoch.  
  *Solution*: We still ran 50 epochs because the model kept improving; early stopping could have been used, but we wanted to see peak performance. Training was executed on the BITS Pilani WILP designated Kubeflow Remote Lab environment.  
- **Sequence length constraints**: Sequence length beyond 128 caused out?of?memory errors on some hardware configurations. We fixed `max_length=128` for training while the model architecture supports up to `max_seq_length=512`.  
...
- **Subword vocabulary limitation**: Because BPE splits words into subword units, word?level manual testing is not meaningful. We therefore rely on the standard MLM validation accuracy computed on subword tokens and on actual validation set examples.
...
- **Subword tokenisation** – word?level evaluation is not directly possible; the model predicts BPE subword units, which is standard for this tokenization approach.
...
- **Loss decreased steadily** without overfitting – validation loss closely tracked training loss.

**Interpretation of accuracy**:  
Random chance on a 10,000?class vocabulary is 0.01%. Our model's 45.7% accuracy is **4,570× better than random**, proving it has learned meaningful subword?level contextual relationships.
